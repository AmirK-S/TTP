...
| `screen_context.relevance` | What of the capture was worth sending: `terms_available`, `terms_sent` (only terms that sound like a dictated word), `continues_sentence` (the text before the cursor ends mid-sentence), `sent`. `sent:false` is the normal case and costs polish nothing. |
