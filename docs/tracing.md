...
| `hotkey.trigger_set` | A trigger the tap now listens for: `slot` is `main` or `second` (the optional second trigger, for an external keyboard whose Fn never reaches the Mac), `trigger` a slug — `fn`, `modifier:54`, `key:49+0x80000`, `mouse:3` — or `null` when the second one is removed. Written at launch and on every change. `hotkey.press` carries the same slug. |
| `hotkey.capture_started` / `hotkey.capture` | Settings asked for a new trigger, and what came of it: `captured` (with the trigger), `rejected` (a bare letter, Caps Lock), `cancelled` (Esc), `timed_out` (15 s — a capture swallows every key, so it cannot be left running), `abandoned` (Settings cancelled). |
| `hotkey.interrupted` | A key was typed while a modifier-only trigger was held, so the modifier was treated as part of a shortcut and released. |
| `hotkey.swallow_tap_armed` / `hotkey.swallow_tap_unavailable` / `hotkey.swallow_tap_rearmed` | The second, active tap that keeps a key or mouse-button trigger from also reaching the focused app. `unavailable` carries `accessibility`: without it the trigger still works but is not swallowed. |
| `paste.deferred` | **This dictation's text waited for the next one's key to come up.** The user started a new dictation while this one was still on its way and was holding a modifier (Fn, ⌘, ⌥) to talk; typing then would have put the text in with that key physically down. `waited_ms` is how long it held back. A `paste.modifiers` line after it means the key was still down when the wait ended. |
| `hotkey.ignored` | A press that did nothing, with the `state` it arrived in — normally `Processing`, the ~0.4 s between release and the pipeline taking the audio. |
| `state.idle_skipped` | **A finishing dictation declined to end a newer recording.** The pipeline hands the hotkey back as soon as it has the audio; its closing `Idle` used to turn the next dictation's `Recording` into `Idle`, reclaiming that capture (`capture.orphan_reclaimed`) while the key was held, and the press after that beeped and recorded nothing (`capture.stop_failed`). |
| `state.transition_dropped` | A pipeline state change lost to a contended lock. A dropped `Idle` parks the app in `Processing`, so this line is the explanation for presses that stop working. |
...
| `screen_context` | What was read from the screen for polish (see `src-tauri/src/screen_context/`). `outcome`: `captured` (with `capture_ms`, `field` = `value` / `range` / `none`, character counts for `before_chars` / `after_chars` / `selected_chars`, `terms`, `window_nodes`, `window_truncated`, `app`), `skipped` with a `reason` (`secure_field`, `excluded_app`, `no_accessibility`…), `pending` (the capture was still running after `waited_ms`) or `missing` (none was started — the setting was off at recording start, or polish is off). Counts only; the block itself is `screen_context.block`, text written only with diagnostics on. `polish.screen_context` says whether the block was sent; `guard_reason: "context_leak"` means the model added words from the screen that were not spoken. |
| `screen_context.relevance` | What of the capture was worth sending: `dictionary_terms` (spellings from the user dictionary, placed first), `screen_captured`, `terms_available`, `terms_sent` (only terms that sound like a dictated word), `continues_sentence` (the text before the cursor ends mid-sentence), `sent`. `sent:false` is the normal case and costs polish nothing. |
| `screen_context.join` | A space was put in front of the pasted text because the cursor sat right after a word. History keeps the text without it. |
...
| `files.swept` | **Stale audio was deleted from disk.** One line per directory (`dir`: `audio_backups` or `recordings`), removing every `.wav` last modified more than 24 hours ago: `count`, `bytes`, and `oldest_age_h`. `trigger:"launch"` always writes both lines, `count:0` included, so a session shows that the sweep ran. `trigger:"hourly"` writes only when something was removed. Before this, the only sweep ran at launch and never looked in `recordings/`, where every exit before `files.cleaned` leaves its working WAV. |
| `cosmetics.state` | **Once per launch: is the Companion (sound packs, the pill's face) unlocked here, and why.** `unlocked`, and `via`: `licence`, `env` (the `TTP_COSMETICS` override maintainer builds carry) or `none`. `licence_status` is the cached licence's raw status (`active`, `expired`, …) or `null` when none was ever activated, so a lapsed licence and "never bought one" do not look alike. `legacy_trial:true` means this install once started the 4-day trial that was removed on 2026-09-11; with `unlocked:false` it explains a supporter-less user whose sounds and face went away. Written off the main thread, because both files it reads are signed and verifying them reads the keychain. |
| `tray.copy_last` | The user picked **Copy last transcription** from the menu-bar icon. `ok:true` carries `chars` and `age_s`, the age of the copied entry in seconds; never the text. `ok:false` carries `reason`: `history_empty` (nothing to copy, which includes history being turned off) or `clipboard_write_failed` with `error`. The entry is the newest one in history, so a dictation that was dropped before `history.saved` cannot be what gets copied. |
...
| `silent_audio` | Below the RMS floor and under 300 ms of speech windows — Whisper was skipped deliberately, to avoid it hallucinating on silence. Since 2026-09-23 the WAV is kept in `audio_backups/` (`backup_kept`) for 24 h: on that date a 46.7 s dictation with long pauses averaged 0.00489 and was deleted. |
