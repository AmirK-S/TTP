...
<task type="auto">
  ...
  <action>
    ...
    7. Wire Rust shortcut events to frontend recording control:
       - shortcuts.rs start_recording() already emits "recording-state-changed" with RecordingState::Recording
       - shortcuts.rs stop_recording() already emits "recording-state-changed" with RecordingState::Idle

       Create src/hooks/useRecordingControl.ts to handle actual recording:
       ```typescript
       ...
       export function useRecordingControl() {
         ...
         useEffect(() => {
           const unlisten = listen<string>('recording-state-changed', async (event) => {
             if (event.payload === 'Recording' && !isRecordingRef.current) {
               isRecordingRef.current = true;
               await startRecording();
             } else if (event.payload === 'Idle' && isRecordingRef.current) {
       ...
       Call this hook in FloatingBar.tsx to wire: Rust shortcut → event → frontend → mic-recorder API
