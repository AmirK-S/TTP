...
1. Import settings::store::get_settings
...
3. Modify setup_shortcuts() to load shortcut from settings:
   - Call get_settings() to get stored shortcut
   - Parse shortcut string or fall back to default Alt+Space
   - Use that shortcut instead of hardcoded one

**lib.rs changes:**
1. Add update_shortcut as a Tauri command export
2. Create wrapper: `#[tauri::command] fn update_shortcut_cmd(app: AppHandle, shortcut: String) -> Result<(), String>`
...
Build succeeds: `cd src-tauri && cargo build`
  ...
  <done>Settings has shortcut field, setup_shortcuts loads from settings, update_shortcut function enables runtime changes</done>
</task>
...
- setup_shortcuts() loads shortcut from settings on startup
